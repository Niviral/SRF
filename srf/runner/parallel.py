...
class ParallelRunner:
    ...
    def __init__(
        self, rotator: SecretRotator, ownership_checker=None, max_workers: int = 5
    ) -> None:
    ...
    def run(
        self, secrets: list[SecretConfig]
    ) -> tuple[list[RotationResult], list[OwnershipResult]]:
        rotation_results: list[RotationResult] = []
        ownership_results: list[OwnershipResult] = []

        logger.info(
            "starting run: %d SP(s), max_workers=%d", len(secrets), self._max_workers
        )
        with ThreadPoolExecutor(max_workers=self._max_workers) as pool:
            rotation_futures = {
                pool.submit(self._rotator.rotate, s): s for s in secrets
            }
            ownership_futures = {}
            if self._ownership_checker is not None:
                ownership_futures = {
                    pool.submit(self._ownership_checker.check_and_update, s): s
                    for s in secrets
                }
            logger.debug(
                "submitted %d rotation + %d ownership task(s)",
                len(rotation_futures),
                len(ownership_futures),
            )

            for future in as_completed(
                list(rotation_futures) + list(ownership_futures)
            ):
                if future in rotation_futures:
                    ...
                    try:
                        ...
                        logger.error(
                            "unexpected runner error for [%s]: %s",
                            secret.name,
                            type(exc).__name__,
                        )
                    ...
                    try:
                        ...
                        logger.warning(
                            "Ownership taks completed partialy for [%s]",
                            secret.name,
                        )
                        ownership_results.append(
                            OwnershipResult(
                                ...
                                app_id=secret.app_id,
                                checked=True,
                                warning="Non-email owners checked and added; email-based owners skipped",
                            )
                        )
                    except Exception as exc:
                        logger.error(
        ...
        logger.info(
            "run complete: %d rotation result(s), %d ownership result(s)",
            len(rotation_results),
            len(ownership_results),
        )
